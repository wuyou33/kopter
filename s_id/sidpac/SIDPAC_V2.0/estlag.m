...
%  Description:
%
%    Estimates the pure time delay of output time series vector y
%    with respect to input time series vector u, using 
...
%  Input:
%    
%      u = input time series vector.  
%      y = output time series vector.
%      t = time vector.
%    trm = steady trim time at the start of the input time series, sec.
%   ldbg = debug output flag
%          = 1 to display debugging output
%          = 0 to skip debugging output
%
%  Output:
%
%    tau = time lag, sec.
%     ul = input time series vector delayed by the estimated lag.
%
%
...
usig=std(u(1:ntrm));
%
%  Find the index where the input time series departs
...
%
%  Compute the smoothed slope for the output time series.
%  Smooth the entire time series to avoid endpoint effects.
%
yd=deriv(y,dt);
%
