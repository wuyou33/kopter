%
%  SWR_DEMO  Demonstrates stepwise regression.  
%
%  Usage: swr_demo;
%
%  Description:
%
%    Demonstrates stepwise regression
...
load 'swr_demo_data.mat'
fprintf('\n\n\n The candidate modeling functions (called regressors) are')
fprintf('\n in the columns of x.  The plot shows these regressors.')
%
%  Set up figure window.
%
FgH=figure('Units','normalized',...
           'Position',[.518 .292 .477 .616],...
           'Color',[0.8 0.8 0.8],...
...
[npts,nr]=size(x);
t=[1:npts]';
plot(t,x),grid on,xlabel('index'),legend('Regressor 1','Regressor 2','Regressor 3','Regressor 4'),
...
fprintf('\n to choose the next candidate regressor to enter the model,')
fprintf('\n then keep only those regressors whose partial F ratio')
...
fprintf('\n     res = measured output z - identified model output y.')
fprintf('\nres_true = measured output z - true model output y_true.')
clf
res=z-y;
