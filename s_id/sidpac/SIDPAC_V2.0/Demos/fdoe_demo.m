%
%  FDOE_DEMO  Demonstrates output-error parameter estimation in the frequency domain.
%
%  Usage: fdoe_demo;
%
%  Description:
...
%    estimation program fdoe.m using noisy data
%    from a supersonic transport simulation. 
%
%  Input:
%
%    None
...
fprintf('\n the supersonic transport simulation using a frequency ')
fprintf('\n sweep input.  The pitch rate response has 10 percent ')
fprintf('\n Gaussian noise added to the values from the simulation.')
...
fprintf('\n\n\n For the frequency domain analysis, use the ')
fprintf('\n frequency vector w = 2*pi*[0.02:0.02:1.0] (rad/sec).')
w=2*pi*[0.02:0.02:1.0]';
fprintf('\n\n Using program loest.m, identify a 1/2 transfer function')
...
fprintf('\n\n Press any key to continue ... '),pause,
fprintf('\n\n The Bode plot shows that the match to the true')
fprintf('\n dynamic system in the frequency domain is excellent.')
...
fprintf('\n oe.m.  This is because the analysis is done in')
fprintf('\n the frequency domain, where the number of data ')
fprintf('\n points is low, and the numerical integrations are')
fprintf('\n replaced with equivalent algebraic vector operations.')
fprintf('\n\n The parameter estimation results and the plots show')
fprintf('\n that results from the output-error formulation')
fprintf('\n in the frequency domain are extremely accurate.')
fprintf('\n\n Press any key to continue ... '),pause,
fprintf('\n\n The Bode plot shows that the match to the true ')
fprintf('\n dynamic system in the frequency domain is excellent.')
