//
//  MEX function runk4.c	 
//
//  MEX file equivalent of runk4.m
//
//  Usage: x = runk4(deqname,p,u,t,x0,c);
//
//  Description:
//
//	  Integrates the differential equations specified
//    in the file named deqname, using fourth-order
//    Runge-Kutta integration with input interpolation.
//
//  Input:
//    
...
//
//  History:  
//    28 Oct 2001 - Created and debugged, EAM.
//
...
// Update the state vector.
    for (j=0;j<ns;j++) {
      xi[j] = x[i+j*npts] + dt*xd2[j]/2;
//      mexPrintf("\n Element %i of xd2 is %f \n", j, xd2[j]);
    }
...
//
    mexCallMATLAB(num_out,out_array,num_in,in_array,deqn);
    xd3=mxGetPr(out_array[0]);
    
// Update the state vector.
    for (j=0;j<ns;j++) {
      xi[j] = x[i+j*npts] + dt*xd3[j];
    }
...
//
    mexCallMATLAB(num_out,out_array,num_in,in_array,deqn);
    xd4=mxGetPr(out_array[0]);
    
//
// Compute the next state.  
//
    for (j=0;j<ns;j++) {
      x[i+1+j*npts] = x[i+j*npts] + dt*(xd1[j]+2*xd2[j]+2*xd3[j]+xd4[j])/6;
    }
  }
  return;
}
