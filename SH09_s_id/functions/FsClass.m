classdef FsClass
    ...
    methods (Static)
        ...
        function [flightData] = importDataFlightTestDataAll(dirWork)
          ...
          for i =1:length(flightData.vars)

            flightData = importDataFlightTestDataSingle(flightData.vars{i}, flightData);
...
function [structureIn] = importDataFlightTestDataSingle(varInStr, structureIn)
  %IMPORTFILE Import numeric data from a text file as column vectors.
  %   CNT_DST_COL = IMPORTFILE(FILENAME) Reads data from text file FILENAME
  %   for the default selection.
  %
  %   CNT_DST_COL = IMPORTFILE(FILENAME, STARTROW, ENDROW) Reads data from
  %   rows STARTROW through ENDROW of text file FILENAME.
  %
  % Example:
  %   CNT_DST_COL = importfile('CNT_DST_COL.csv',2, 3829743);
  %
  %    See also TEXTSCAN.

  % Auto-generated by MATLAB on 2018/05/21 21:39:59

  %% Initialize variables.
  delimiter = {''};
  startRow = 2;
  endRow = inf;
  filename = [varInStr '.csv'];
  ...
  vector = cell2mat(raw(:, 1));
  structureIn.(varInStr) = vector;
