function [cost,y,p] = simplex(dsname,pi,u,t,x0,c,del,z,rr,fid,p0,M0)
%
%  SIMPLEX  Simplex method for parameter optimization without cost gradients.  
%
%  Usage: [cost,y,p] = simplex(dsname,pi,u,t,x0,c,del,z,rr,fid,p0,M0);
%
%  Description:
%
...
%
pt(:,1)=pi;
[cpt(1),y]=compcost(dsname,pi,u,t,x0,c,z,rr,p0,M0);
del=abs(del);
for j=1:np,
  pt(:,j+1)=pi;
  if abs(pi(j))~=0.0
  ...
  pt(j,j+1)=pi(j) + dp;
  [cpt(j+1),y]=compcost(dsname,pt(:,j+1),u,t,x0,c,z,rr,p0,M0);
end
[maxcost,ihi]=max(cpt);
[mincost,ilo]=min(cpt);
...
%
  ...
  [cptr,y]=compcost(dsname,ptr,u,t,x0,c,z,rr,p0,M0);
%
%  If the reflection gives the lowest cost yet, try an extension.
%
  if cptr < cpt(ilo)
    ptrx=pavg + rxc*(ptr-pavg);
    [cptrx,y]=compcost(dsname,ptrx,u,t,x0,c,z,rr,p0,M0);
%
...
%
        for j=1:np+1,
          if j~=ilo
            ...
            [cpt(j),y]=compcost(dsname,pt(:,j),u,t,x0,c,z,rr,p0,M0);
          end
        end
      end
    end
...
p=pt(:,ilo);
[cost,y]=compcost(dsname,p,u,t,x0,c,z,rr,p0,M0);
return
